...
/**
 * Polls the {@link NativeTrack} timestamp, if the platform supports it, taking care of polling at
...
/* package */ final class AudioTimestampPoller {
  ...
  /**
   * NativeTrack timestamps are deemed spurious if they are offset from the system clock by more than
   * this amount.
   *
  ...
  private final NativeTrackPositionTracker.Listener errorListener;
   ...
   *
   * @param nativeTrack The audio track that will provide timestamps.
   * @param errorListener The {@link NativeTrackPositionTracker.Listener} for timestamp errors.
   */
  public AudioTimestampPoller(
      NativeTrack nativeTrack, NativeTrackPositionTracker.Listener errorListener) {
    this.audioTimestamp = new AudioTimestampWrapper(nativeTrack);
    this.sampleRate = nativeTrack.getOriginalSampleRate();
   ...
   * transition and reusing of the {@link NativeTrack}.
   */
  ...
  private static final class AudioTimestampWrapper {
     ...
     *
     * <p>When an {@link NativeTrack} is reused during offloaded playback, the {@link
     ...
     * @param nativeTrack The audio track that will provide timestamps.
     */
    public AudioTimestampWrapper(NativeTrack nativeTrack) {
    ...
    public boolean maybeUpdateTimestamp() {
      boolean updated = nativeTrack.getTimestamp(audioTimestamp);
      if (updated) {
