...
/**
 * Wraps an {@link NativeTrack}, exposing a position based on {@link
 * NativeTrack#getPlaybackHeadPosition()} and {@link
 * NativeTrack#getTimestamp(android.media.AudioTimestamp)}.
 *
 * <p>Call {@link #start()} immediately before calling {@link NativeTrack#play()}. Call {@link
 ...
 */
/* package */ final class NativeTrackPositionTracker {
  ...
  /**
   * NativeTrack latencies are deemed impossibly large if they are greater than this amount.
   *
  ...
  private float nativeTrackPlaybackSpeed;
   ...
   *
   * <p>When an {@link NativeTrack} is reused during offloaded playback, rawPlaybackHeadPosition is
   * reset upon track transition. {@link NativeTrackPositionTracker} must be notified of the
   ...
   */
  public NativeTrackPositionTracker(
      Listener listener,
      Clock clock,
    ...
    audioTimestampPoller = new AudioTimestampPoller(nativeTrack, listener);
    outputSampleRate = nativeTrack.getOriginalSampleRate();
   ...
   *
   * @param nativeTrackPlaybackSpeed The playback speed.
   */
  public void setNativeTrackPlaybackSpeed(float nativeTrackPlaybackSpeed) {
    this.nativeTrackPlaybackSpeed = nativeTrackPlaybackSpeed;
  ...
  public long getCurrentPositionUs(long writtenFrames) {
    NativeTrack nativeTrack = checkNotNull(this.nativeTrack);
    if (nativeTrack.getPlayState() == PLAYSTATE_PLAYING) {
      maybeSampleSyncParams();
    }

    // If the device supports it, use the playback timestamp from NativeTrack.getTimestamp.
    ...
    long positionUs =
        useGetTimestampMode
            ? audioTimestampPoller.getTimestampPositionUs(systemTimeUs, nativeTrackPlaybackSpeed)
    ...
    int nativeTrackPlayState = nativeTrack.getPlayState();
    if (nativeTrackPlayState == PLAYSTATE_PLAYING) {
      ...
      if (lastSystemTimeUs != C.TIME_UNSET) {
        ...
        long expectedPositionDiffUs =
            getMediaDurationForPlayoutDuration(elapsedSystemTimeUs, nativeTrackPlaybackSpeed);
    ...
    } else if (nativeTrackPlayState == PLAYSTATE_STOPPED) {
  ...
  /** Starts position tracking. Must be called immediately before {@link NativeTrack#play()}. */
  public void start() {
  ...
  public boolean isPlaying() {
    return checkNotNull(nativeTrack).getPlayState() == PLAYSTATE_PLAYING;
  }
   ...
   * {@link NativeTrack} and an impending track transition.
   */
  ...
  private void maybeTriggerOnPositionAdvancingCallback(long positionUs) {
    ...
    long playoutDurationSinceLastPositionUs =
        Util.getPlayoutDurationForMediaDuration(
            mediaDurationSinceResumeUs, nativeTrackPlaybackSpeed);
  ...
  private void maybeSampleSyncParams() {
    ...
    if (systemTimeUs - lastPlayheadSampleTimeUs >= MIN_PLAYHEAD_OFFSET_SAMPLE_INTERVAL_US) {
      ...
      if (playbackPositionUs == 0) {
        // The NativeTrack hasn't output anything yet.
        return;
      }
      // Take a new sample and update the smoothed offset between the system clock and the playhead.
      playheadOffsets[nextPlayheadOffsetIndex] =
          Util.getPlayoutDurationForMediaDuration(playbackPositionUs, nativeTrackPlaybackSpeed)
              - systemTimeUs;
  ...
  private boolean maybeUpdateLatency(long systemTimeUs) {
    ...
    if (isOutputPcm
      ...
      try {
        ...
        latencyUs = checkNotNull(nativeTrack).latency() * 1000L - bufferSizeUs;
        ...
        // The method existed, but doesn't work.
      }
  ...
  private long getPlaybackHeadPositionEstimateUs(long systemTimeUs) {
    ...
    if (playheadOffsetCount == 0) {
      // The NativeTrack has started, but we don't have any samples to compute a smoothed position.
      positionUs =
          stopTimestampUs != C.TIME_UNSET
      ...
      positionUs =
          Util.getMediaDurationForPlayoutDuration(
              systemTimeUs + smoothedPlayheadOffsetUs, nativeTrackPlaybackSpeed);
    }
  ...
  /**
   * {@link NativeTrack#getPlaybackHeadPosition()} returns a value intended to be interpreted as an
  ...
  private long getSimulatedPlaybackHeadPositionAfterStop() {
    if (checkNotNull(this.nativeTrack).getPlayState() == AudioTrack.PLAYSTATE_PAUSED) {
      // If NativeTrack is paused while stopping, then return cached playback head position.
    ...
    long mediaTimeSinceStopUs =
        Util.getMediaDurationForPlayoutDuration(elapsedTimeSinceStopUs, nativeTrackPlaybackSpeed);
  ...
  private void updateRawPlaybackHeadPosition(long currentTimeMs) {
    NativeTrack nativeTrack = checkNotNull(this.nativeTrack);
    int state = nativeTrack.getPlayState();
    if (state == PLAYSTATE_STOPPED) {
      // The audio track hasn't been started. Keep initial zero timestamp.
      return;
    }
    long rawPlaybackHeadPosition = 0xFFFFFFFFL & nativeTrack.getPlaybackHeadPosition();
    if (SDK_INT <= 29) {
      if (rawPlaybackHeadPosition == 0
          && this.rawPlaybackHeadPosition > 0
          && state == PLAYSTATE_PLAYING) {
        // If connecting a Bluetooth audio device fails, the NativeTrack may be left in a state
